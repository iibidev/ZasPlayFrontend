...
const $$ = (el: any) => document.querySelectorAll(el);
...
function showPicError(text: any){
...
form.addEventListener("submit", async(evt: any)=>{
    evt.preventDefault();
    ...
    try {
        const response = await fetch(env.BACKURL + "/auth/register", {
            method: "POST",
            headers: {
  ...
  goBack(){
    const pages = document.querySelector(".form_pages") as HTMLElement;
    const nextBtn = document.querySelector(".form__next") as HTMLElement;
    const submitBtn = document.querySelector(".form__submit") as HTMLElement;
    pages.scrollLeft = 0;
  ...
  charsLeft(){
    const usernameChars = document.querySelector(".usernameChars") as HTMLElement;   
    const username = document.querySelector("#username") as HTMLInputElement;
    const chars = username.value.length;
    usernameChars.textContent = (16 - chars).toString();
    if(chars > 16){
